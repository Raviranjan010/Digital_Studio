...
This project now includes an Express backend for contact form delivery and reusable section content APIs.

```bash
cp .env.example .env
```

Fill in `SMTP_USER` and `SMTP_PASS` with your Gmail address and a Gmail App Password, then run:
...
Contact submissions are saved to `backend/storage/contact-messages.json`. When SMTP is configured, the backend sends:

- a notification email to `raviranjan01b@gmail.com`
- a styled thank-you reply to the sender in the same black and gold theme as the website
